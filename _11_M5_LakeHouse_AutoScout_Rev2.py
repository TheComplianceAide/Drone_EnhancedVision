#!/usr/bin/env python3
"""
M5 LakeHouse AutoScout Rev2 for DJI/Mavic RTMP.
...
Rev2 goal: at least 25% better field autonomy by using a stronger event mask,
water/wake-specific scoring, and smarter target ranking while keeping the same
simple AUTO/FIREWORKS/WAVE/BIRDS buttons.
...
from m5_v2_core import event_mask_v2, lake_signal_boost_v2, select_track_v2


LIVE_NAME = "M5 LakeHouse Live"
CONSOLE_NAME = "M5 LakeHouse AutoScout Rev2"
...
def _measure_lake(gray: np.ndarray, diff: np.ndarray, mask_bool: np.ndarray, zones: LakeZones) -> LakeMetrics:
    ...
    boost = lake_signal_boost_v2(gray, diff, mask_bool, zones)

    burst_score = _clamp(
        (sky_event * 75.0)
        + (sky_bright * 8.0)
        + max(0.0, sky_p99 - 10.0) / 32.0
        + boost.burst * 0.28,
        0.0,
        1.0,
    )
...
def _pick_target(tracks: Sequence[PulseTrack], tuning: LakeTuning, zones: LakeZones) -> Optional[PulseTrack]:
    if not tracks:
        return None
    frame_h = max(zones.water_start + 80, zones.shore_end + 120, 240)
    return select_track_v2(
        tracks,
        (frame_h, frame_h),
        focus=tuning.focus,
        zones=(zones.sky_end, zones.shore_end, zones.water_start),
    ) or max(tracks, key=lambda tr: _track_score(tr, tuning, zones))
    ...
    try:
        while True:
            ...
            event_v2 = event_mask_v2(
                ...
                diff,
                threshold=threshold,
                glint_factor=provisional.glint_factor,
            )
