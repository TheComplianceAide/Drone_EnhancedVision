#!/usr/bin/env python3
"""
MacBook M-series field preset Rev2 for the Radar Motion + AutoZoom script.
...
Rev2 goal: at least 25% better field reliability by choosing a profile from the
actual device result, keeping RTMP latency tight, and making dry-run output show
the resolved inference budget before takeoff.
"""
...
PROFILE_CHOICES = ["auto", *sorted(PROFILES)]
...
def _apply_capture_env() -> None:
    ...
    os.environ.setdefault(
        ...
        "fflags;nobuffer|flags;low_delay|probesize;32|analyzeduration;0|rw_timeout;900000",
    )
...
def _parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Apple Silicon Rev2 preset wrapper for _07 Radar Motion + AutoZoom."
    )
...
def _resolve_profile_v2(requested: str, chosen_device: str) -> tuple[str, int, int, str]:
    if requested != "auto":
        w, h = PROFILES[requested]
        return requested, w, h, "requested"
    if chosen_device == "mps":
        w, h = PROFILES["detail"]
        return "detail", w, h, "auto: MPS won benchmark"
    if chosen_device == "cpu":
        w, h = PROFILES["balanced"]
        return "balanced", w, h, "auto: CPU path avoids GPU transfer tax"
    w, h = PROFILES["low-latency"]
    return "low-latency", w, h, "auto: benchmark inconclusive"
...
def main() -> int:
    ...
    probe_profile = "balanced" if args.profile == "auto" else args.profile
    profile_w, profile_h = PROFILES[probe_profile]
    probe_w = int(args.infer_w or profile_w)
    probe_h = int(args.infer_h or profile_h)
    ...
    resolved_profile, profile_w, profile_h, profile_why = _resolve_profile_v2(args.profile, chosen_device)
    ...
    print(
        f"[M5 preset Rev2] {_hardware_label()} | profile {resolved_profile} "
        f"{infer_w}x{infer_h} ({profile_why}) | device {chosen_device} ({why})",
        flush=True,
    )
