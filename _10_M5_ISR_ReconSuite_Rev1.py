#!/usr/bin/env python3
"""
M5 ISR Recon Suite for DJI/Mavic RTMP.
...
def _lucky_stack(stack: LuckyStack, bgr: np.ndarray, *, enabled: bool, alpha: float = 0.18) -> Tuple[np.ndarray, str]:
    ...
    a = float(_clamp(alpha, 0.06, 0.42))
    stack.accum = (1.0 - a) * stack.accum + a * aligned
    stack.gray = (1.0 - a) * stack.gray + a * gray
    stack.quality = 0.92 * stack.quality + 0.08 * min(1.0, response * 2.0)
    return np.clip(stack.accum, 0, 255).astype(np.uint8), f"stack {stack.quality:.2f}"
...
def main() -> int:
    ...
    try:
        while True:
            ...
            glint_mask = bright & (diff > max(4.0, threshold * glint_factor))
